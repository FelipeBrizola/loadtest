...
class HttpClient {
	...
	makeRequest() {
		...
		if (typeof this.params.requestGenerator == 'function') {
			request = this.params.requestGenerator(this.params, this.options, lib.request, this.getConnectHttp(id, requestFinished, this.params.contentInspector));
		} else {
			if (this.params.protocol === 'http2') {
				lib = http2.connect(`${this.options.protocol}//${this.options.host}`);
				request = lib.request(Object.assign(this.options.headers, { ':method': this.options.method, ':path': this.options.path }));
				const conn = this.getConnectHttp2(id, lib, requestFinished, this.params.contentInspector)
				conn(request);
			}
			else
				request = lib.request(this.options, this.getConnectHttp(id, requestFinished, this.params.contentInspector));
		}
	...
	getConnectHttp(id, callback, contentInspector) {
		let body = '';
		return connection => {
	 ...
	 */
	getConnectHttp2(id, client, callback, contentInspector) {
		let body = '';
		return connection => {
			log.debug('HTTP client connected to %s with id %s', this.params.url, id);
			connection.setEncoding('utf8');
			...
			connection.on('response', (headers) => {
				const result = {
					host: connection.sentHeaders[':authority'],
					path: connection.sentHeaders[':path'],
					method: connection.sentHeaders[':method'],
					statusCode: headers[':status'],
					body,
					headers: connection.sentHeaders.headers
				};
				if (contentInspector) {
					contentInspector(result)
				}
