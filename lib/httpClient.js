...
class HttpClient {
	...
	makeRequest() {
		...
		if (typeof this.params.requestGenerator == 'function') {
			...
			process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
			lib = http2.connect('https://localhost:3000');
			// lib = http2.connect(this.options.href);
			request = lib.request({ ':method': 'GET', ':path': '/health' });
			const conn = this.getConnect(id, lib, requestFinished, this.params.contentInspector)
			conn(request);
	 ...
	 */
	getConnect(id, client, callback, contentInspector) {
		let body = '';
		return connection => {
